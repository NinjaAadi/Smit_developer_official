...
exports.likePost = async(req,res,next) =>{
   try {
        ...
        post.dislikes = post.dislikes.filter(user => user.user.toString() != req.user._id.toString());
...
exports.deletecomment = async (req,res,next) => {
  try {
    ...
    post.comments = post.comments.filter(cmt => cmt._id.toString()!=commentid.toString()
    )
    await post.save();
    res.status(200).json({
      success:true
